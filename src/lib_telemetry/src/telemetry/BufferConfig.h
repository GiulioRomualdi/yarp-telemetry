...
#ifndef TELEMETRY_BUFFER_CONFIG_H
#define TELEMETRY_BUFFER_CONFIG_H
...
/**
 * @brief Struct containing the parameters for configuring a telemetry::BufferManager.
 *
 */
struct  BufferConfig {
    ...
    bool auto_save{ false };/**< the flag for enabling the save in the destructor of the telemetry::BufferManager */
...
/**
 * @brief Populate the telemetry::BufferConfig struct reading from a json file.
 *
 * @param[out] bufferConfig The struct to be filled in.
 * @param[in] config_filename The name of the json file.
 * @return true on success, false otherwise.
 */
bool bufferConfigFromJson(telemetry::BufferConfig& bufferConfig, const std::string& config_filename);

/**
 * @brief Save on a json file the content of a telemetry::BufferConfig struct.
 *
 * @param[in] bufferConfig The struct to be saved to file.
 * @param[in] config_filename The name of the json file to be saved.
 * @return true on success, false otherwise.
 */
bool bufferConfigToJson(const telemetry::BufferConfig& bufferConfig, const std::string& config_filename);
